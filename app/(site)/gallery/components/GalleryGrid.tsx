...
type GalleryEvent = Awaited<ReturnType<typeof getGalleryEvents>>[number];
...
export default function GalleryGrid({ events }: { events: GalleryEvent[] }) {
  const [activeCategory, setActiveCategory] = useState('all');
  const [lightbox, setLightbox] = useState<LightboxState | null>(null);

  const categories = [
    { label: 'All', value: 'all' },
    ...Array.from(new Set(events.map((e) => e.category))).map((value) => ({
